...
//!    (src/term_reset/mod.rs, the `--reset-terminal` rescue) is the
...
/// honored only inside `src/term_reset/mod.rs` — the `--reset-terminal`
...
fn source_tree_has_only_sanctioned_dec_modes() {
    ...
    assert!(
        ...
        "NIGHT-improve-7 violation: non-sanctioned DEC private mode \
         ...
         7, 1003, 1015}} is honored ONLY in src/term_reset/mod.rs, and \
...
/// `src/term_reset/mod.rs` (NIGHT-hunt-31).
fn scan_source_file(path: &Path, offenders: &mut Vec<String>) {
    ...
    let is_reset_contract = path.ends_with("term_reset/mod.rs");
    let needle = "\\x1b[?";
