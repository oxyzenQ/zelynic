...
use crate::systemd_wrapper::failure_simulation::{
    build_failure_simulation_matrix, render_failure_simulation_result, simulate_failure_scenario,
    CleanupDecision, FailureScenario, PidLocationStatus, RollbackDecision, SimulationResult,
    PHASE_LABEL,
};
...
fn all_pid_location_labels_are_covered() {
    let locations: std::collections::HashSet<_> = matrix().iter().map(|r| r.pid_location).collect();
