...
#
#   ./scripts/rust-version-to.sh 1.99.0
#
...
# `scripts/check-rust-version-sync.sh`, which runs as the final gate.
#
# USAGE:
...
EXAMPLES:
    ...
    ./scripts/rust-version-to.sh --check 1.98.1   # Verify current pin
...
audit_docs() {
	...
	if [[ "${stale_count}" -gt 0 ]]; then
		...
		log_warn "by hand, then rerun: ./scripts/rust-version-to.sh --check ${new_full}"
	else
...
run_check_mode() {
	...
	if bash "${SYNC_CHECK}" >/dev/null 2>&1; then
		...
		log_err "Sources out of sync — rerun: ./scripts/rust-version-to.sh ${target_full}"
