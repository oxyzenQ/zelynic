...
if [[ -n "$IFACE" ]]; then
	...
	# Check zelynic still runs with the interface down (must not crash)
...
# Check no BPF programs remain (informational; reload follows)
bpftool prog show 2>/dev/null | grep -c "enforce" || true
...
ORPHAN_PINS=$(find /sys/fs/bpf/zelynic -mindepth 1 -maxdepth 1 2>/dev/null | wc -l || true)
