# Changelog — v10 Era (frozen history)

This file is the frozen release history of zelynic's pre-v11 era,
moved verbatim from CHANGELOG.md in NIGHT-improve-4 so the active
changelog stays small. The sections below (development versions
[1.0.0] through [7.0.0]) document the release line that preceded the
v11.0.0 API-stability and maintenance-mode contract — the v10 era.
They are historical records and are never rewritten; the same
frozen-record policy that excludes CHANGELOG.md from the header and
disclaimer gates applies here.
